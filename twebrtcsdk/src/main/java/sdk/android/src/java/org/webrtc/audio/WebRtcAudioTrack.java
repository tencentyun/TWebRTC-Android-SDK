...
class WebRtcAudioTrack {
  ...
  // corresponding to voice communications, such as telephony or VoIP. //weifei: change to music
  private static final int DEFAULT_USAGE = getDefaultUsageAttribute();

  private static int getDefaultUsageAttribute() {
    if (Build.VERSION.SDK_INT >= 21) {
      return AudioAttributes.USAGE_MEDIA;//weifei: change to USAGE_MEDIA
    } else {
  ...
  private static AudioTrack createAudioTrackOnLollipopOrHigher(
    ...
    // Create an audio track where the audio usage is for VoIP and the content type is speech. weifei: change to CONTENT_TYPE_MUSIC
